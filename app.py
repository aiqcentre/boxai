...
FORBIDDEN_SQL_KEYWORDS = (
    "delete","update","insert","alter","drop","truncate","create", "replace","grant","revoke","attach","detach", "copy","load","export","pragma","call", "vacuum","set",
)
DESTRUCTIVE_INTENT_WORDS = (
    "delete","remove","drop","truncate","update","insert","modify","change","alter","create","add column","erase",
)
    ...
    if is_au_query(question):
        with st.spinner("Answering from AU City/State data..."):
            try:
                ...
                final_answer = (
                    str(getattr(au_res, "data", au_res))
                    if au_res is not None
                    else "No answer."
                )
                sql = "(n/a for AU tool)"
                df = pd.DataFrame()
    ...
    # ---------- NL → SQL pipeline ----------
        ...
        with st.spinner("Generating SQL & running..."):
            ...
            rows_for_llm = json.loads(
                df.head(ANSWER_ROWS_LIMIT).to_json(
